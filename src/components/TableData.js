  ...
  return (
    <tbody>
      {props.info && props.info.map((element, index) => {
        if (props.select === "All") {
          return (
            <tr key={index}><td>{element.carpark}</td><td>{element.category}</td><td>{element.weekdays_rate_1}</td><td>{element.weekdays_rate_2}</td><td>{element.saturday_rate}</td><td>{element.sunday_publicholiday_rate}</td></tr>
          )
